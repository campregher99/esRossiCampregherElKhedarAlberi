...
/**
 * Classe che rappresenta un albero di elementi, ossia un grafo aciclico non
...
public class Tree {
	...
	/**
	 * Costruttore vuoto, privato. Si limita ad istanziare un albero vuoto.
	 */
	private Tree() {
	...
	/**
	 * Costruttore <b>CASUALE</b> che genera un nuovo albero dalla dimensione
	 * prefissata, attingendo dalla lista di stringhe passata come parametro per
	 * generare le etichette dei nodi. <b>Nota:</b> questo metodo fa in modo che
	 * tutte le etichette passate nella lista vengano utilizzate nella generazione
	 * dell'albero. Questo assicura che l'albero conterrà sicuramente almeno un
	 * nood per ciascuna etichetta della lista. <b>Nota:</b> per scelta
	 * implementativa, il nodo radice NON VIENE CONTATO nella dimensione
	 * dell'albero.
	 * 
	...
	/**
	 * Verifica se all'interno dell'albero è presente almeno un nodo con la label
	 * specificata.
	 * 
	...
	/**
	 * Verifica se nei nodi derivanti dal nodo passato c'� o no un nodo che si
	 * chiama come label
	 * 
	 * @param nodo  () (nodo di partenza)
	 * @param label (nome del nodo da trovare)
	 * @return boolean (true se c'�, false se non lo trova)
	 */
	private boolean isTrovato(Node nodo, String label) {
		if (nodo.isTrovato(label)) {
			return true;
		} else if (nodo.getNumberOfAdjacents() == 0) {
			return false;
		} else {
